...
const Header: React.FC = () => {
  ...
  return (
    <header className="bg-gradient-primary text-white font-bold relative">
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navLinks.map((link) => (
            <a key={link.name} href={link.href} className="text-lg hover:text-brand-light-cyan transition-colors">
              {link.name}
            </a>
          ))}
        </nav>
      ...
      {isMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-gradient-primary z-20">
          <nav className="flex flex-col items-center space-y-4 py-6">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} className="text-lg hover:text-brand-light-cyan transition-colors" onClick={() => setIsMenuOpen(false)}>
                {link.name}
              </a>
            ))}
          </nav>
